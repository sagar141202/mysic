export default function NowPlaying() {
  return (
    <div style={{
      ...
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 600, color: 'var(--text-secondary)', letterSpacing: '0.05em', marginBottom: '24px', textTransform: 'uppercase', fontSize: '11px' }}>Now Playing</h2>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '6px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginTop: '4px' }}>The Weeknd</p>
        </div>
        <button style={{
          background: 'none', border: 'none', fontSize: '20px',
          color: 'var(--accent-pink)', cursor: 'pointer', paddingTop: '2px',
        }}>♡</button>
      </div>
      ...
      <div style={{ margin: '20px 0 8px' }}>
        <div style={{
          ...
          <div style={{
            width: '38%', height: '100%', borderRadius: '3px',
        ...
        {['⇄', '⏮', null, '⏭', '↻'].map((ctrl, i) => {
          ...
          return (
            <button key={i} style={{
              ...
              height: isPlay ? 52 : 36,
              borderRadius: '50%',
              background: isPlay ? 'var(--accent-grad)' : 'var(--glass-bg)',
              border: isPlay ? 'none' : '1px solid var(--glass-border)',
              color: 'white', fontSize: isPlay ? '20px' : '16px',
              cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
              transition: 'all 0.2s ease',
              boxShadow: isPlay ? '0 8px 24px rgba(168,85,247,0.4)' : 'none',
